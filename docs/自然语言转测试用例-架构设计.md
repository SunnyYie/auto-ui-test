...
### 3.3 图片预览组件测试（yet-another-react-lightbox）

**场景：** maimai-community-pc 图片预览迁移，8 个测试用例覆盖打开/切换/关闭/样式/手势防护

**架构集成模式：** 混合策略在组件级测试中的应用

```text
utils/fixture.js (aiFixture + stealth)
        ↓ 提供 { page, ai }
  baseTest.extend({ page: CDN ORB route fix })  ← Fixture 扩展模式
        ↓
adapter.js (executeInstruction)
        ↓ 按操作类型分层
┌──────────────────────────────────────────────┐
│ 适配器层: 点击/导航/等待/验证               │
│   → Playwright 选择器优先 + AI 兜底          │
├──────────────────────────────────────────────┤
│ Playwright 原生层: CSS 样式断言/坐标操作     │
│   → evaluate() / mouse.click() / expect()    │
│   → AI 无法检查 computedStyle，必须原生 API  │
└──────────────────────────────────────────────┘
```
...
| 操作                 | 执行方式                   | 原因                        |
| -------------------- | -------------------------- | --------------------------- |
| 缩略图/按钮点击      | adapter `handleClick`      | Playwright 优先 + AI 兜底   |
| 页面导航             | adapter `handleNavigate`   | Playwright `page.goto()`    |
| 文本验证             | adapter `handleVerify`     | 关键词提取 + AI 兜底        |
| CSS 计算样式断言     | Playwright `evaluate()`    | AI 无法操作 `computedStyle` |
| DOM 样式属性断言     | Playwright `evaluate()`    | 精确值对比                  |
| 坐标点击（背景关闭） | Playwright `mouse.click()` | 需要 boundingBox 计算       |
| DOM 遍历查询         | Playwright `evaluate()`    | gallery 图片计数            |

**关键技术点：**

1. **Fixture 扩展** — 在 `utils/fixture.js` 的 AI fixture 基础上通过 `baseTest.extend()` 追加 CDN ORB 路由拦截
2. **适配器直接调用** — 不走 `runWorkflow()`（避免 LLM 调用开销），直接使用 `executeInstruction()` 构造指令
3. **分层断言** — 交互操作通过适配器（获得 AI 兜底能力），样式断言用 Playwright 原生（精确可靠）
...
5. **统一 Fixture** — 所有测试文件使用 `utils/fixture.js` 提供的 `{ page, ai }` 上下文，确保 ZeroStep AI 兜底能力全局可用

### 4.6 测试架构统一模式

所有测试文件遵循统一的架构模式，确保 ZeroStep AI 兜底能力贯穿每个测试场景：

```text
 测试类型 A: 全流程测试 (login, baidu-search)
   fixture.js → { page, ai } → runWorkflow(prompt, { page, ai })
                                       ↓
                              LLM 解析 → adapter 执行 → 结果

 测试类型 B: 需求→用例管线 (case-gen)
   fixture.js → { page, ai } → generateCases() → runTestCase(case, { page, ai })
                                                          ↓
                                                  caseToPrompt → runWorkflow

 测试类型 C: 组件级精确测试 (image-preview)
   fixture.js → { page, ai } → executeInstruction(instruction, { page, ai })
                                       ↓
                              adapter 直接执行（跳过 LLM 解析）
                                       +
                              Playwright 原生 API（精确断言）
```

**类型 C 的设计理念：** 对于 UI 组件的精确测试，很多断言（CSS 样式、DOM 属性、坐标操作）AI 无法完成，需要 Playwright 原生 API。但交互操作（点击按钮、导航、验证文本）仍通过适配器执行，保留 AI 兜底能力。这种 "适配器 + 原生混合" 是组件级测试的最佳实践。
